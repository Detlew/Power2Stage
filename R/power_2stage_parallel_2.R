...
#
# author D.L.
# this is a variant in which simulations (mean/variances) are done for
# the combined stage 1 + 2
# -----------------------------------------------------------------------------
# require(PowerTOST)
# source("./R/sampsiz2.R")
...
power.2stage.p2 <- function(method=c("B","C"), alpha0=0.05, alpha=c(0.0294,0.0294), 
...
# ----- stage 1 ----------------------------------------------------------
  ...
  if(length(pes_tmp)>0){
    ...
    # ----- simulate stage 1+2 data ------
    # may be this gives better numerical stability (no negative res. error in ANOVA)
    nsim2 <- length(pes_tmp)
    # means T & R
    nT   <- n1T+n2T
    nR   <- n1R+n2R
    mT   <- ifelse(n2T>0, rnorm(n=nsim2, mean=mlog, sd=sqrt(varT/nT)), m1T)
    mR   <- ifelse(n2T>0, rnorm(n=nsim2, mean=0,    sd=sqrt(varR/nR)), m1R)
    # point est.
    pes  <- mT-mR
    # total mean
    mtot <- (nT*mT + nR*mR)/(nT+nR)
    # means for s1, s2 (stages)
    m1  <- (n1T*m1T + n1R*m1R)/(n1T+n1R)
    # stage 2
    m2  <- ((nT+nR)*mtot - (n1T+n1R)*m1)/(n2T+n2R)
    # do we need to recalculate? paranoia
    n1  <- n1T + n1R
    n2  <- n2T + n2R
    # simulate variances via chi-squared distribution
    # stay with stage 1 if n2X == 0
    varsT  <- ifelse(n2T>0, varT*rchisq(n=nsim2, df=nT-1)/(nT-1), varsT)
    varsR  <- ifelse(n2R>0, varR*rchisq(n=nsim2, df=nR-1)/(nR-1), varsR)
    ...
    if (test=="t-test" || test=="anova"){
      ...
      if (test=="anova"){
        ...
        Vpooled <- ifelse(n2R+n2T>0,
                          (dfs*Vpooled - n1*(m1-mtot)^2 - n2*(m2-mtot)^2)/(dfs-1),
                          Vpooled)
    ...
    BE[is.na(BE)]   <- BE2
    # done with them
    rm(BE2, nts, lower, upper, hw, n2T, n2R)
